...
const CheckOutLot = () => {
    ...
    const handleConfirmButtonPress = async () => {
        try {
            ...
            // within the database to ensure it exists and it is able to be checked out
            for (const barcode of pendingBarcodes) {
                ...
                // Revieve the correct key for the computer and update its status to false to ensure that it is checked out
                if (snapshot.exists()) {
                    computerFound = true;
                    const computerKey = Object.keys(snapshot.val())[0];
                    await databaseRef.child(computerKey).update({ available: false });
                } else {
                    alert(`Lot(s) "${data}" not found/available in the inventory.`);
                }
            }

            //
            if (computerFound) {
                navigation.navigate('checkOutDetails', { scannedBarcodes });
            } else {
                setTimeout(() => {
                    alert('No lots were checked out.');
                }, 3000)
            }
