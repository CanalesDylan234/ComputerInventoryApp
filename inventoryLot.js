...
const LotinventoryPage = () => {
    ...
    const handleConfirmButtonPress = async () => {
        ...
        try {
            const databaseRef = firebase.database().ref('lots');
            ...
            for (const barcode of pendingBarcodes) {
                ...
                if (computerExists) {
                    alert(`Lots(s) "${data}" already exist(s) in the inventory. Please empty your array before adding.`); 
                } else {
    ...
    const checkIfComputerExists = async (computerName) => {
        ...
        const snapshot = await firebase.database().ref('lots').orderByChild('data').equalTo(computerName).once('value');
    ...
    const handleDeleteButtonPress = async () => {
        try {
            ...
            if(computerKey) {
                ...
                alert(`The Lot(s) "${pendingBarcodes[0].data}" has been deleted from the database!`);
                navigation.navigate('inventoryDeletion'); 
            } else {
                alert(`The Lot(s) "${pendingBarcodes[0].data}" was not found in the inventory.`);
            }
        } catch (error) {
            console.error("Error deleting lot from database", error);
            alert("Error deleting lot from database, error");
        }
    };
                ...
                <View>
                    ...
                    <View>
                        <TouchableOpacity style={styles.scanButton} onPress={handleCheckStoredBarcodes}>
                            <Text style={styles.buttonText}>Check Current Scanned Lots!</Text>
                        </TouchableOpacity>
                    </View>

                    <View>
                        <TouchableOpacity style={styles.scanButton} onPress={handleClearScannedBarcodes}>
                            <Text style={styles.buttonText}>Need to restart? Clear all scanned Lots!</Text>
                        </TouchableOpacity>
                    </View>

                    <View>
                        <TouchableOpacity style={styles.scanButton} onPress={handleDeleteButtonPress}>
                            <Text style={styles.buttonText}>Delete Lots from inventory!</Text>
                        </TouchableOpacity>
                    </View>
